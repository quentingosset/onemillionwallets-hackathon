...
class StateContainerState extends State<StateContainer> {
  ...
  BaseTheme curTheme = AvalancheTheme();
  ...
  String selectedNetworkAddAccount = "43114";
  ...
  void handleSubscribeResponse(SubscribeResponse response) {
    ...
    setState(() {
      ...
      if (response.balance == null) {
        wallet.accountBalance = double.tryParse("0");
      } else {
        wallet.accountBalance = BigInt.tryParse(response.balance);
      }
      wallet.nanoPrice = "1";
      wallet.btcPrice = "0.00001662";
  ...
  Future<void> handleCallbackResponse(CallbackResponse resp) async {
    ...
    if (receivedHash != null) {
      AccountHistoryResponseItem histItem = AccountHistoryResponseItem(
        ...
        contract_ticker_symbol: resp.account,
        amount: resp.amount,
        contract_name: receivedHash
      );
      if (!wallet.history.contains(histItem)) {
        setState(() {
          wallet.history.insert(0, histItem);
          wallet.accountBalance += double.tryParse(resp.amount);
  ...
  Future<void> _requestBalances(Map<String, AccountHistoryResponse> account_history) async {
    ...
    //Map<String, AccountsBalancesHistoryResponse> resp = await sl.get<AccountService>().requestBalanceHistory(accounts);

    double globalValue = 0;

    accounts.forEach((account) {
      double value = 0;
      account_history.entries.toList().where((element) => element.key.toUpperCase() == account.address.toUpperCase()+""+account.network.toString().toUpperCase()).toList()[0].value.history.forEach((element) {
        value += element.quote;
      });
      if (double.parse(value.toString()) != double.parse(account.balance?? "0")) {
        sl.get<DBHelper>().updateAccountBalance(account, value.toString());
      }
      globalValue += value;
    });
    wallet.accountBalance = double.tryParse(globalValue.toString());
  ...
  Future<void> requestUpdate({bool pending = true}) async {
    if (wallet != null &&
        wallet.address != null) {
      EventTaxiImpl.singleton().fire(SubscribeEvent(response: SubscribeResponse()));
      ...
      /*sl.get<AccountService>().queueRequest(SubscribeEvent(response: new SubscribeResponse({})));
      sl.get<AccountService>().processQueue();*/
      ...
      try {
        walletsMap.clear();
        List<Account> accounts = await sl.get<DBHelper>().getAccounts();
        walletsMap.addEntries(accounts.map((e) => MapEntry(e.address+e.network.toString(), AppWallet(network: e.network.toString(), address: e.address))));
        Map<String, AccountHistoryResponse> resp = await sl.get<AccountService>().requestAccountHistory(accounts, count: count);
        _requestBalances(resp); // TODO
        bool postedToHome = true;
        // Iterate list in reverse (oldest to newest block)
        resp.entries.forEach((resp) {
          for (AccountHistoryResponseItem item in resp.value.history) {
            item.network = resp.value.network;
            item.walletAddress = resp.value.account;
  ...
  void requestAccountHistory(String account) {
    /*sl.get<AccountService>().queueRequest(AccountHistoryRequest(account: account, count: 100), fromTransfer: true);
    sl.get<AccountService>().processQueue();*/
  }
  ...
  Future<String> _getPrivKey() async {
    return Util.seedToPrivate(await sl.get<Vault>().getSeed(), selectedAccount.index);
  }
