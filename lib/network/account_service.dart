...
import 'package:kalium_wallet_flutter/network/model/request/contract_transaction_history_request.dart';
...
import 'model/request/actions.dart';
...
const String _SERVER_ADDRESS_HTTP = "https://api.covalenthq.com/v1/{NETWORK}/address/{ADDRESS}/{SCOPE}";
...
class AccountService {
  ...
  Future<void> initCommunication({bool unsuspend = false}) async {
    ...
    try {
      ...
      EventTaxiImpl.singleton().fire(SubscribeEvent(response: SubscribeResponse())); // delete when use outside server
  ...
  Future<dynamic> makeHttpRequest(BaseRequest request) async {
    ...
    http.Response response = await http.get(
      _SERVER_ADDRESS_HTTP
          .replaceAll('{NETWORK}', request.account.network.toString())
          .replaceAll('{ADDRESS}', (request.action == Actions.ACCOUNTS_BALANCES)? request.account.address : request.account.address)
          .replaceAll('{SCOPE}', request.action)
          .replaceAll('{CONTRACT_ADDRESS}', (request.action == Actions.CONTRACT_TRANSACTION_HISTORY)? request.params.toLowerCase() : ""),
        headers:  {
          'Content-type': 'application/json'
        },
        //body: json.encode(request.toJson())
    ...
    /*if (decoded.containsKey("error") && decoded.entries["error"].toString() == true) {
      return ErrorResponse.fromJson(decoded);
    }*/
    return decoded;
  }
  ...
  Future<Map> requestAccountHistory(List<Account> accounts, { int count = 1}) async {
    Map<String, AccountHistoryResponse> listAccountHistory = Map();
    for(var account in accounts) {
      ...
      response["data"]["items"].removeWhere((token) => token['type'] == "dust" || (BigInt.parse(token['balance']) / BigInt.from(10).pow(token['contract_decimals']) < 1));
      listAccountHistory.addAll({account.address+""+account.network.toString(): AccountHistoryResponse.fromJson(response["data"])});
    }
    return listAccountHistory;
  }
  Future<Map> requestBalanceHistory(List<Account> accounts, { int count = 1}) async {
    Map<String, AccountsBalancesHistoryResponse> listAccountHistory = Map();
    for(var account in accounts) {
      AccountsBalancesHistoryRequest request = AccountsBalancesHistoryRequest(
          account: account
      );
      dynamic response = await makeHttpRequest(request);
      if (response is ErrorResponse) {
      ...
      //(BigInt.parse(token['balance']) / BigInt.from(10).pow(response["data"]["items"][8]['contract_decimals']) < 0.0001)
      response["data"]["items"].removeWhere((token) => token['type'] == "dust" || BigInt.parse(token['balance']) == 0);
      //listAccountHistory.add(AccountHistoryResponse.fromJson(response["data"]));
      listAccountHistory.addAll({account.address+""+account.network.toString(): AccountsBalancesHistoryResponse.fromJson(response["data"])});
      //listAccountHistory.add(account.address+""+account.network.toString(), () => AccountHistoryResponse.fromJson(response["data"]));
   }
    return listAccountHistory;
  }

  Future<TransactionList> requestContractTransactionHistory(Contract contract) async {
    TransactionList listTransaction;
    ContractTransactionHistoryRequest request = ContractTransactionHistoryRequest(
        account: Account(address: contract.address, network: contract.network),
        count: contract.network,
    ...
    listTransaction = TransactionList.fromJson(response["data"]["items"].map((e) {return e["transfers"][0];}).toList());
