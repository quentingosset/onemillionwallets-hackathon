...
class NumberUtil {
  ...
  ///
  static Decimal getRawAsUsableDecimalWithMaxDecimal(String raw,int maxDecimal) {
    Decimal amount = Decimal.parse(raw.toString());
    Decimal result = amount / Decimal.parse(BigInt.from(10).pow(maxDecimal).toString());
    return result;
  }
  ...
  /// @returns 1
  ///
  static String getRawAsUsableStringMaxDecimal(String raw, int maxDecimal) {
    NumberFormat nf = new NumberFormat.currency(locale:'en_US', decimalDigits: maxDecimalDigits, symbol:'');
    String asString = nf.format(truncateDecimal(getRawAsUsableDecimalWithMaxDecimal(raw,maxDecimal)));
