...
class AppWallet {
  ...
  AppWallet({String address, String network, BigInt accountBalance, String frontier, String openBlock, String representativeBlock,
    ...
    this._network = network ?? "43114";
    this._accountBalance = accountBalance ?? 0.0;
  ...
  String getLocalCurrencyPrice({String locale = "en_US"}) {
    double converted = _accountBalance;
    switch (locale) {
      case "es_VE":
  ...
  String getLocalCurrencyPriceAmount({String locale = "en_US", double amount}) {
    Decimal converted = Decimal.parse(amount.toString());
    switch (locale) {
      case "es_VE":
        return NumberFormat.currency(locale:locale, symbol: "Bs.S").format(converted.toDouble());
      case "tr_TR":
        return NumberFormat.currency(locale:locale, symbol: "₺").format(converted.toDouble());
      default:
        return NumberFormat.simpleCurrency(locale:locale,decimalDigits: 3).format(converted.toDouble());
    }
  }
  ...
  String get btcPrice {
    double converted = double.tryParse(_btcPrice) * double.tryParse(_accountBalance.toString());
    // Show 4 decimal places for BTC price if its >= 0.0001 BTC, otherwise 6 decimals
    if (converted >= double.tryParse("0.0001")) {
