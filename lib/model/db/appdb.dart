...
class DBHelper{
  ...
  initDb() async {
    ...
    String path = join(documentsDirectory.path, "onemillionwallets.db");
    print(path);
  ...
  Future<List<Account>> getAccounts() async {
    ...
    for (int i = 0; i < list.length; i++) {
      accounts.add(Account(id: list[i]["id"], name: list[i]["name"], index: list[i]["acct_index"], lastAccess: list[i]["last_accessed"], selected: list[i]["selected"] == 1 ? true : false,address: list[i]["address"], balance: list[i]["balance"]?? "0", network: list[i]["network"]));
    }
  ...
  Future<List<Account>> getRecentlyUsedAccounts({int limit = 2}) async {
    ...
    List<Account> accounts = new List();
    for (int i = 0; i < list.length; i++) {
      accounts.add(Account(id: list[i]["id"], name: list[i]["name"], index: list[i]["acct_index"], lastAccess: list[i]["last_accessed"], selected: list[i]["selected"] == 1 ? true : false, balance: list[i]["balance"], address: list[i]["address"]));
    }
    /*for (Account a in accounts) {
      a.address = NanoUtil.seedToAddress(
        await sl.get<Vault>().getSeed(),
  ...
  Future<Account> addAccount({String nameBuilder}) async {
    ...
    await dbClient.transaction((Transaction txn) async {
      ...
      account = Account(index: nextIndex, name:nextName, lastAccess: 0, selected: false, address: Util.seedToAddress(await sl.get<Vault>().getSeed(), nextIndex));
  ...
  Future<int> saveAccount(Account account) async {
    var dbClient = await db;
    return await dbClient.rawInsert('INSERT INTO Accounts (name, acct_index, last_accessed, selected, address, network) values(?, ?, ?, ?, ?, ?)', [account.name, account.index, account.lastAccess, account.selected ? 1 : 0, account.address, account.network]);
  }
  ...
  Future<Account> getSelectedAccount() async {
    ...
    Account account = Account(id: list[0]["id"], name: list[0]["name"], index: list[0]["acct_index"], selected: true, lastAccess: list[0]["last_accessed"], balance: list[0]["balance"],  address: list[0]["address"]);
    return account;
  }
  ...
  Future<Account> getMainAccount() async {
    ...
    String address = Util.seedToAddress(await sl.get<Vault>().getSeed(), list[0]["acct_index"]);
